...
function isItAnApple(fruitArray) {
  for (i=0; i< fruitArray.length; i++) {
    console.log(fruitArray[i] === "apple");
  }
}
...
function getCarInfoByIndex(inventory, index) {
  const selection = inventory[index];
  console.log(`This is a ${selection.car_make} ${selection.car_model}`);
}


getCarInfoByIndex( inventory, 10);
/**
...
*/
function getModelYears(carsWithYears) {
  year = [];
  for (i = 0; i < carsWithYears.length; i++) {
    year.push(carsWithYears[i].car_year);
  }
  return year
}
// console.log(cars[0].car_year);
